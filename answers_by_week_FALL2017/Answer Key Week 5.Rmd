...
Setting ```convert = TRUE``` will solve the problem above: the years will automatically be converted back to numbers with ```gather```.
...
Control what happens when there are too many or too few pieces to separate. By default both give warnings. Alternatively, extra levels can be dropped or merged. Too few levels can be filled by adding NA to the right or the left of the levels that are present. When you specify these, you don't get warnings, which is nice.
...
Using ```count``` we can see the number of occurences of unique combinations of ```country```, ```iso2```, and ```iso3```. We can see that no ```country```, ```iso2```, or ```iso3``` is shown in more than one combination.
