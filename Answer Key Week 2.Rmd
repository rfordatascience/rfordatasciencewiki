...
They make faceted plots, but only with a single column in the case of ```drv ~ .``` and a single row in the case of ```. ~ cyl```. The dot tells R that there is not facetting variable on that side of the formula.

### 3.5.1.4

It's easier to identify the clusters in the facets. The disadvantage is that it is hard to see the overall effect of engine size (```displ```) on mileage (```hwy```). When you have a really large data set with overlapping clusters, it will be hard to see the individual clusters. That would be a reason to use facetting.
